...
function getDeviceId(){
  ...
  if(!id){
    ...
    localStorage.setItem("uy_device_id",id);
  }
  return id;
}
...
function ThemeToggle({
  ...
  return(
    <div className="inline-flex rounded-lg border border-white/30 overflow-hidden text-xs shrink-0">
      {options.map(o=>(
        <button
          key={o.id}
          type="button"
...
function GetTheApp(){
  ...
  return(
    <div className="relative mt-5 bg-white dark:bg-slate-900 rounded-2xl p-5 shadow border border-blue-100 dark:border-slate-700">
      <button
        type="button"
        onClick={dismiss}
...
function AppFooter(){
  return(
    <footer className="text-center text-sm font-semibold text-slate-600 dark:text-slate-200 py-6 px-4">
...
const MEMBER_DRAW_POLL_MS=500;
const MEMBER_DRAW_RESULT_VISIBLE_MS=60000;
const DRAW_ANIMATION_CYCLE_MS=700;

function getDrawAnimationDelay(startedAt:string|null){
  if(!startedAt)
    return "0ms";
...
export default function App(){
  ...
  const activeDrawStartedAtRef =
    useRef<string|null>(
      null
    );

  const completedDrawStartedAtRef =
    useRef<string|null>(
      null
    );

  const drawAnimationDelayRef =
    useRef("0ms");
  ...
  const pollMemberDraw=async(
    ...
    try{
      ...
      if(
        ...
        const startedAt=
          state.draw.startedAt;

        if(
          startedAt&&
          startedAt!==
            activeDrawStartedAtRef.current
        ){
          activeDrawStartedAtRef.current=
            startedAt;

          completedDrawStartedAtRef.current=
            null;

          drawAnimationDelayRef.current=
            getDrawAnimationDelay(
              startedAt
            );
      ...
      /*
       * Draw polling is independent from normal dashboard polling.
       * A temporary failure must not log the member out.
       */
    }
  };
  ...
  return(
    <div className="min-h-screen bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100">
      ...
      <section className="bg-blue-50 dark:bg-slate-900 p-6 text-center">
        <div className="max-w-xl mx-auto">
          <div className="h-48 flex flex-col justify-center">
            <h1 className="text-3xl font-black text-blue-800 dark:text-blue-300">
              {
                slides[
                  slide
                ].title
                ...
                <div className="text-center mt-4">
                  ...
                  {showForgot&&
                    <p className="text-sm text-slate-500 dark:text-slate-300 mt-2">
                      There's no self-service password reset yet. Contact an administrator directly (call, message, or in person) and they can reset it for you.
                    </p>
                  }
                </div>
                ...
                <Panel title="Personal information">
                  ...
                  <label className="block mb-4">
                    ...
                    <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">
                      Email is the default and doesn't cost anything to send. Pick SMS only if you'd rather get the code as a text message.
                    </p>
                  </label>
              ...
              {step===2&&
                ...
                >
                  ...
                  <p className="text-xs text-slate-400 dark:text-slate-400 mt-3 text-center">
                    Didn't get your OTP? Contact an admin directly - they can generate a fresh code for you.
                  </p>
                </Panel>
              }
              ...
              {step===4&&
                <Panel title="Rules Lock Area">
                  ...
                  >
                    ...
                    I have read and agree to the Unique Youth rules.
                  </label>
...
function Panel({
  ...
  return(
    <div className="border dark:border-slate-700 rounded-2xl shadow-sm p-5 dark:bg-slate-900">
...
function MemberRollingDice({
  ...
  return(
    <div className="mt-5 rounded-2xl overflow-hidden border-2 border-red-200 dark:border-red-900 shadow-sm">
      {rolling&&
        <div className="bg-red-50 dark:bg-red-950/20 p-8 text-center">
          <div className="relative w-28 h-28 mx-auto flex items-center justify-center">
            <div className="absolute w-28 h-28 rounded-full bg-red-100 dark:bg-red-950/40 animate-ping opacity-50"/>

            <div
              className="relative w-24 h-24 rounded-3xl bg-red-600 text-white flex items-center justify-center shadow-xl animate-[memberDiceRoll_0.7s_ease-in-out_infinite]"
              style={{
                animationDelay
              }}
            >
          ...
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400 max-w-md mx-auto">
            Two eligible members are being randomly selected to receive this month's lump-sum.
          </p>
      ...
      {completed&&
        <div className="bg-green-50 dark:bg-green-950/30 p-6 text-center">
          ...
          <h2 className="mt-2 text-xl font-black text-green-700 dark:text-green-300">
            Two members have been selected
          </h2>

          <p className="mt-1 text-sm text-green-700/80 dark:text-green-300/80">
            The monthly lump-sum recipients have been selected. The draw notice will disappear after one minute.
          </p>
        </div>
      }
    </div>
...
function Dashboard({
  ...
  const isVerified=
    status==="awaiting_slot_assignment"||
    status==="active";
        ...
        :(
          <main className="max-w-6xl mx-auto p-5">
            ...
            {!isActive&&
              status&&
              <div
                className="mt-5 bg-white dark:bg-slate-900 rounded-2xl p-6 shadow border-l-4 border-red-600"
                style={
                  STATUS_FONT
                }
              >
            ...
            {isActive&&
              <>
                ...
                {mp&&
                  <div className="mt-6 bg-white dark:bg-slate-900 rounded-2xl p-5 shadow">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <h2 className="text-xl font-bold text-slate-900 dark:text-white">
                        This month's contribution target
                      </h2>
                ...
                {dashboard?.lateFee&&
                  <div
                    className={`mt-5 rounded-2xl p-5 shadow border-l-4 ${
                  ...
                  >
                    ...
                    <p className="text-slate-600 dark:text-slate-300 mt-1">
                      ...
                      }{" "}
                      {
                        new Date(
                          dashboard.lateFee.status==="paid"
                            ?dashboard.lateFee.paidAt
                            :dashboard.lateFee.imposedAt
                        ).toLocaleDateString(
                          undefined,
                          {
                            year:
                    ...
                    {dashboard.lateFee.status!=="paid"&&
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">
                        This is separate from your monthly contribution. Pay via bank transfer like usual and send proof to the admin — they'll mark it paid here.
                      </p>
                    }
                  </div>
                }
    ...
    <main className="max-w-3xl mx-auto p-5">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 shadow flex items-center gap-5">
        {avatarPreview
          ...
          :(
            <div className="w-24 h-24 rounded-full bg-blue-800 text-white flex items-center justify-center text-3xl font-black">
              {
                (
                  u.firstName||
                  "?"
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">
            JPG or PNG. It'll be resized automatically.
          </p>
        </div>
      </div>
          ...
          <tbody>
            {rows.map(
              ([
                ...
                >
                  <td className="p-4 font-semibold text-slate-500 dark:text-slate-300 w-1/3 align-top">
                    {label}
                  </td>
